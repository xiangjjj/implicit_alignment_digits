...
class TaskSampler(metaclass=Singleton):
    ...
    def get_sampling_frequency(self):
        if self.args.self_train is True:
            freq = 2
        elif self.args.source_sample_mode and self.args.target_sample_mode:
            freq = 2
        else:
            freq = 1
        return freq
        ...
        for _ in range(self.max_iter):
            ...
            if len(batch) == 0:
                for cls in classes:
                    ...
                    if self.args.sample_without_replacement is True:
                        self.remove_list_of_examples_from_pseudo_label_dict(cls, samples_for_this_class)
